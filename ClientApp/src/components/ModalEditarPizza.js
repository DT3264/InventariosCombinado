...
export default function ModalEditarPizza(props) {
  ...
  const updatePizza = async (data) => {
    ...
    if (data.operacion === "elimina") {
      await axios.delete(`pizza/${data.id}`, headers);
    } else if (data.operacion === "actualiza") {
      await axios.post(`pizza/updatePizza`, pizza, headers);
    } else if (data.operacion === "agregar") {
      await axios.post(`pizza`, pizza, headers);
    }
    return;
  };
  ...
  let content =
    props.formData.operacion === "elimina" ? (
      <>
        ...
        <Form.Group>
          ...
          <Form.Select
            ...
            onChange={(e) =>
              props.setFormData({
                ...props.formData,
                [e.target.name]: Array.from(e.target.selectedOptions, (item) =>
                  parseInt(item.value)
                ),
              })
            }
